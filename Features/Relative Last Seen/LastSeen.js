// NEVKO LastSeen — replaces Steam's fuzzy "1 ч. и 42 мин. назад" labels
// with exact timestamps: "Был(а) в сети: 13:20"
//
...
;(function nevkoLastSeen() {
  ...
  // Extract m_rtLastSeenOnline from any persona-like object
  ...
  function getTimestampFromFiber(el) {
    ...
    while (fiber && depth < 40) {
      ...
      if (p && typeof p === "object") {
        // Common prop names Steam uses across different UI contexts
        const ts =
  ...
  // Attribute set on elements we own — used to suppress characterData re-entry
  const OWNED_ATTR = "data-nevko-ls"

  // WeakSet of elements currently being written to — guards the characterData path
  const writing = new WeakSet()

  // WeakSet of fully patched elements — exit early on re-visit
  const patched = new WeakSet()

  function setOwnedText(el, text) {
    // Mark as writing so the characterData handler ignores this mutation
    writing.add(el)
    el.textContent = text
    // writing flag is cleared after the current microtask finishes
    // (MutationObserver callbacks run as microtasks, so this is safe)
  ...
  const observer = new MutationObserver(mutations => {
    for (const m of mutations) {
      if (m.type === "childList") {
        ...
        // Ignore mutations caused by our own setOwnedText writes
  ...
  // Observer is started once and never disconnected — no race conditions
  function init() {
    patchAllLabels()
    observer.observe(document.body, {
      childList: true,
      subtree: true,
      // On web Steam injects friend spans empty and fills text in a separate mutation,
      // so we need characterData to catch the fill. The observer handler already ignores
      // our own writes via the `writing` WeakSet, so there is no feedback loop.
      characterData: true,
    })
  }
